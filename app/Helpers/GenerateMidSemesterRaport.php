...
class GenerateMidSemesterRaport
{
    public static function make($id, $semester, $term, $formattedDate)
    {
        ...
        $data_nilai = Grading::where('term_id', $term_id)->where('semester_id', $semester_id)->whereIn('learning_data_id', $learning_data)->get();
        ...
        foreach ($nilai_akhir as $nilai) {
            ...
            $key = "nilai_akhir_term_{$term_id}_semester_{$semester_id}";
        ...
        if (isset($data_nilai_akhir_semester_1)) {
        ...
        } elseif (isset($data_nilai_akhir_semester_2)) {
            $data_nilai_akhir_total = $data_nilai_akhir_semester;

            // Isi data hanya dari semester 2
            foreach ($data_nilai_akhir_semester_2 as $member_id => $pembelajaran_data) {
                foreach ($pembelajaran_data as $pembelajaran_id => $data_semester_2) {
                    if (!isset($data_nilai_akhir_total[$member_id])) {
                        $data_nilai_akhir_total[$member_id] = [];
                    }

                    $data_nilai_akhir_total[$member_id][$pembelajaran_id] = [
                        'nilai_akhir_total' => $data_semester_2['nilai'],
                        'nama_mapel' => $data_semester_2['nama_mapel'],
                        'nama_mapel_indonesian' => $data_semester_2['nama_mapel_indonesian'],
                        'kkm' => $data_semester_2['kkm'],
                        'deskripsi_nilai' => $data_semester_2['deskripsi_nilai'],
                        'semester_id' => 2, // Karena ini hanya semester 2
                        'predikat' => $data_semester_2['predikat'],
                        'nilai_akhir_term_1_semester_1' => null,
                        'nilai_akhir_term_2_semester_1' => null,
                        'nilai_akhir_term_1_semester_2' => $data_semester_2['nilai'],
                        'nilai_akhir_term_2_semester_2' => null,
                        'nilai_akhir_semester_1' => null,
                        'nilai_akhir_semester_2' => $data_semester_2['nilai']
                    ];
                }
            }
        } else {
            ...
            foreach ($data_nilai_akhir_semester as $member_id => $pembelajaran_data) {
                foreach ($pembelajaran_data as $pembelajaran_id => $data_semester) {
                    if (!isset($data_nilai_akhir_total[$member_id])) {
                        $data_nilai_akhir_total[$member_id] = [];
                    }
                }
        ...
        $view = 'print.print-mid-semester-raport';
